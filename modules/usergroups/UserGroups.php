...
?>
	<tr>
		...
		<td id="group-color-row">
			<p>
				<input type="text" name="user-group[group-color]" id="group-color" class="color-field" value="<?php echo self::get_meta('group-color'); ?>" />
...
<?php  } else { ?>
		<div class="form-field">
			<p>
				<input type="text" style="width:40%" name="user-group[group-color]" id="group-color" class="color-field" value="<?php echo self::get_meta('group-color'); ?>" />
			</p>
		</div>
		<div id="color-picker" style="z-index: 100; background:#eee; border:1px solid #ccc; position:absolute; display:none;"></div>
	<?php
...
?>
		...
		<script type="text/javascript">
			jQuery(document).ready(function($) {
				...
				$('#bulkeditusergroupform').on('submit', 'form', function() {
